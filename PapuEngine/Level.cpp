...
glm::vec2 Level::getRandomFreePosition()
{
	int posRandom = rand() % freeSpaces.size() + 1;
	glm::vec2 randomPos = freeSpaces.at(posRandom);
	freeSpaces.erase(freeSpaces.begin() + posRandom);
	return randomPos;
}
	...
	for (size_t y = 0; y < levelData.size(); y++)
	{
		for (size_t x = 0; x < levelData[y].size(); x++)
		{
			...
			case 'Z':
				...
				break;
			case '.':
				freeSpaces.push_back(glm::vec2(x*TILE_WIDTH, y* TILE_WIDTH));
				break;
			default: 
				break;
			}
